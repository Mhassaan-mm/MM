...
class ApixuClient:
    def __init__(self, api_key, api_url=API_URL, lang=None):
        self.api_key = api_key
        self.api_url = api_url.rstrip('/')
        self.lang = lang

    def _get(self, url, args,q):
        new_args = {}
        if self.api_key:
            new_args['key'] = self.api_key
        new_args.update(args)
        response = requests.get(url, params={'access_key':self.api_key,'query':q}, timeout=HTTP_TIMEOUT)
        res = response.json()
        if 'error' in res:
            err_msg = res['error'].get('info')
